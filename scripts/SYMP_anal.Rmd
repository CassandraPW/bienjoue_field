...
SympTest <- read_csv("data/SympTest.csv")
...
#Outcome measures
score<-aggregate(CR~ID,symp,  sum) # The number of correct button presses
time <- aggregate(rt~ rang+ID, symp, sum)
score <- merge(score, time)
score$speed <- (score$CR * 60000) / score$rt #Measure of speed (Correct responses * 60000) / time to complete the activity

#Measure the distance effect, on the x-axis put the distance between the compared numbers and on the y-axis the RT
time <- aggregate(rt~ rang+ID+dist, symp, sum)
...
write.csv(time, 'data/clean/cleanSYMPData.csv', row.names = FALSE)
```
...
cor <- merge(cor, score[,c('rang', 'speed')])
colnames(cor)[3]<-'numbercomparisonSpeed'
cor
